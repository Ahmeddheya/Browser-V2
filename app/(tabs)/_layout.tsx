...
export class StorageManager {
  ...
  static async exportData(): Promise<{ history: HistoryItem[]; bookmarks: BookmarkItem[]; settings: BrowserSettings }> {
    try {
      const [history, bookmarks, settings] = await Promise.all([
        this.getHistory(),
        this.getBookmarks(),
        this.getSettings(),
      ]);
  ...
  static async importData(data: { history?: HistoryItem[]; bookmarks?: BookmarkItem[]; settings?: Partial<BrowserSettings> }): Promise<void> {
    try {
      if (data.history) {
        await this.setItem(STORAGE_KEYS.HISTORY, data.history);
      }
      if (data.bookmarks) {
        await this.setItem(STORAGE_KEYS.BOOKMARKS, data.bookmarks);
      }
      if (data.settings) {
        const currentSettings = await this.getSettings();
        await this.setItem(STORAGE_KEYS.SETTINGS, { ...currentSettings, ...data.settings });
      }
    } catch (error) {
